...
# Función para leer palabra de 16 bits
def read_word(reg):
...
alpha_fusion = 0.96  # Equilibrio entre gyro (preciso) y acc (estable)
...
try:
    while True:
        ...
        if abs(az_f) < 0.01:  # evitar división por cero
            roll_acc = roll
        else:
