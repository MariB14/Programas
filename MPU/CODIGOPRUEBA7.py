...
bus = smbus.SMBus(1)
addr = 0x68
bus.write_byte_data(addr, 0x6B, 0)  # Activar el MPU6050
...
try:
    while True:
        ...
        # Calcular posición angular en grados
        angle_x += gx_f * dt
        angle_y += gy_f * dt
        angle_z += gz_f * dt

        # Imprimir cada 10 ciclos para evitar saturación
        if ciclo % 10 == 0:
            print(f"Aceleración: X={ax_f:.2f}g  Y={ay_f:.2f}g  Z={az_f:.2f}g")
            print(f"Ángulo estimado: X={angle_x:.1f}°  Y={angle_y:.1f}°  Z={angle_z:.1f}°\n")
